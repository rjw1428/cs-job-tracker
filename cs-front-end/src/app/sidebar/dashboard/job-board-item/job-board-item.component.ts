...
export class JobBoardItemComponent implements OnInit {
  ...
  onDueDateSelected() {
    this.dialog.open(UpdateDueDateComponent, {
      ...
      .subscribe(updatedJob => {
        if (updatedJob)
          this.backendSerivce.saveData('updateDueDate', updatedJob)
      })
  }
  ...
  onStartEndDateSelected() {
    this.dialog.open(AwardTimelineFormComponent, {
      ...
      .subscribe(updatedJob => {
        if (updatedJob)
          this.backendSerivce.saveData('updateTimeline', updatedJob)
      })
  }
  ...
  onDiscount() {
    this.dialog.open(AddFinalPriceComponent, {
      ...
      .subscribe(finalCost => {
        if (finalCost)
          this.backendSerivce.saveData('setFinalCost', { finalCost, job: this.job })
      })
  }
